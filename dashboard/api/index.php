...
switch ($c){
	...
	// series connection log
	case "series_connection_log":
		series_connection_log();
		break;
...
function series_connection_log()
{
	global $conn;
...
function headend()
{
	...
	if($query !== FALSE) {
		...
		foreach($headends as $headend) {
			...
			$query = $conn->query("SELECT * FROM `streams` WHERE `server_id` = '".$headend['id']."' AND `stream_type` = 'input' ");
			if($query !== FALSE) {
				// $output[$count]['streams'] = $query->fetchAll(PDO::FETCH_ASSOC);

				// get outputs
				/*
				foreach($output[$count]['streams'] as $input_stream){
					$query = $conn->query("SELECT * FROM `streams` WHERE `source_stream_id` = '".$input_stream['id']."' ");
					$output[$count]['streams']['output'][] = $query->fetchAll(PDO::FETCH_ASSOC);
				}
				*/
				...
				if(is_array($streams)){
					foreach($streams as $stream){
						$output[$count]['streams'][$stream_count] = $stream;

						$query = $conn->query("SELECT * FROM `streams` WHERE `source_stream_id` = '".$stream['id']."' ");
						$output[$count]['streams'][$stream_count]['output'] = $query->fetchAll(PDO::FETCH_ASSOC);
						$stream_count++;
					}
			...
			if($query !== FALSE) {
				...
				foreach($channels as $channel) {
					...
					$output[$count]['channels'][$channel_count]['transcoding_profile_id'] 	= $channel['transcoding_profile_id'];
			...
			// get vod watch folders
			$query = $conn->query("SELECT * FROM `vod_watch` WHERE `server_id` = '".$headend['id']."' ");
			if($query !== FALSE) {
				$output[$count]['vod_watch_folders'] = $query->fetchAll(PDO::FETCH_ASSOC);
				$output[$count]['total_vod_watch_folders'] = count($output[$count]['vod_watch_folders']);

				// get existing vod so no dupe omdb check
				$query = $conn->query("SELECT `file_location` FROM `vod` WHERE `server_id` = '".$headend['id']."' ");
				$output[$count]['vod_files'] = $query->fetchAll(PDO::FETCH_ASSOC);
				

				/*if(is_array($vod_file_locations)){
					foreach($vod_file_locations as $vod_file_location){
						$output[$count]['vod_files'][] = $vod_file_location['file_location'];
					}
				}else{
					$output[$count]['vod_files'] = array();
				}
				*/
			}else{
				$output[$count]['total_vod_watch_folders'] = 0;
			}
...
function stream_status_update()
{
	...
	$update = $conn->exec("UPDATE `streams` SET `status` = '".$stream_status."' WHERE `source_stream_id` = '".$stream_id."' ");
	...
	if($stream_status == 'online'){
		...
		$stream_uptime			= get('uptime');
		$stream_fps				= get('fps');
		$stream_speed			= get('speed');
		...
		if(isset($_GET['aspect_ratio']) && !empty($_GET['aspect_ratio'])){
			$stream_aspect_ratio 	= get('aspect_ratio');
...
function stream_info_client()
{
	...
	// check if customer is allowed to access this stream
	$stream_allowed = false;
	$query 			= $conn->query("SELECT `bouquet_id` FROM `bouquets_content` WHERE `content_id` = '".$stream_id."' ");
	$bouquets 		= $query->fetchAll(PDO::FETCH_ASSOC);

	foreach($bouquets as $bouquet){
		if(in_array($bouquet['bouquet_id'], $customer['bouquet'])){
			$stream_allowed = true;
		}
	}
	
	if($stream_allowed == false){
		$data['status'] = 'error';
		$data['message'] = 'access to this stream is not allowed';
	...
	if($streams_found != 0) {
		...
		// get node details
		$query 					= $conn->query("SELECT `id`,`wan_ip_address`,`http_stream_port` FROM `headend_servers` WHERE `id` = '".$stream['server_id']."' ");
...
function stream_out_headend_info()
{
	...
	$query = $conn->query("SELECT `id`,`wan_ip_address`,`public_hostname`,`http_stream_port` FROM `headend_servers` WHERE `id` = '".$server_id."' ");
	$headend = $query->fetch(PDO::FETCH_ASSOC);
...
function series_info_client()
{
	...
	if($series_found != 0) {
		...
		// get headend info
		$query 						= $conn->query("SELECT `id`,`wan_ip_address`,`http_stream_port` FROM `headend_servers` WHERE `id` = '".$series['server_id']."' ");
...
function vod_info_client()
{
	...
	if($vod_found != 0) {
		...
		// get headend info
		$query 					= $conn->query("SELECT `id`,`wan_ip_address`,`http_stream_port` FROM `headend_servers` WHERE `id` = '".$vod['server_id']."' ");
...
function channel_info_client()
{
	...
	if($channel_found != 0) {
		...
		// get headend info
		$query 						= $conn->query("SELECT `id`,`wan_ip_address`,`http_stream_port` FROM `headend_servers` WHERE `id` = '".$channel['server_id']."' ");
...
function vod_watch_add()
{
	...
	$full_path 		= base64_decode($full_path);
	...
	if(isset($server) && isset($server['id'])){
		...
		$query = $conn->query("SELECT `id` FROM `vod` WHERE `file_location` = '".$full_path."' ");
			...
			if(in_array($file_ext, $allowed_files)){
				...
				if(isset($metadata) && $metadata['status'] == 'match'){
					...
					$insert = $conn->exec("INSERT INTO `vod` 
				        (`user_id`,`server_id`,`name`,`file_location`,`description`,`cover_photo`,`year`,`genre`,`runtime`,`language`)
				        VALUE
				        (
				        	'1',
					        '".$server_id."',
					        '".addslashes($metadata['name'])."',
					        '".$full_path."',
					        '".addslashes($metadata['description'])."',
					        '".addslashes($metadata['cover_photo'])."',
					        '".addslashes($metadata['year'])."',
					        '".addslashes($metadata['genre'])."',
					        '".addslashes($metadata['runtime'])."',
					        '".addslashes($metadata['language'])."'
					    )");
				}else{
					// add new record
					$insert = $conn->exec("INSERT INTO `vod` 
...
function streams_slave()
{
	...
	if($query !== FALSE) {
		...
		foreach($headends as $headend) {
			...
			// get streams to transcode
			$query = $conn->query("SELECT * FROM `streams` WHERE `server_id` = '".$headend['id']."' AND `id` = '".$stream_id."' ");
			if($query !== FALSE) {
				// $output[$count]['streams'] = $query->fetchAll(PDO::FETCH_ASSOC);
